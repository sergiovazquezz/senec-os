...
static void gdt_create()
{
    ...
    const uint32_t tss_limit = sizeof(tss_t) - 1;

    gdt.tss_low = tss_descriptor_low(tss_base, tss_limit);
    gdt.tss_high = tss_descriptor_high(tss_base);
}
...
void gdt_init()
{
    ...
    const gdtr_t gdtr = { .limit = sizeof(gdt_t) - 1, .address = (uint64_t)&gdt };
